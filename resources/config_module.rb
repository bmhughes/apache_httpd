...
action :create do
  if module_disabled?
    Chef::Log.warn('Module is disabled, skipping creation')
    return
  end
...
action :enable do
  converge_by("Enable site configuration for #{new_resource.name}") do
    ...
    when 'debian'
      shell_out!("/usr/sbin/a2enmod #{new_resource.name}")
    end
  end unless module_enabled?
end
...
action :disable do
  converge_by("Disable site configuration for #{new_resource.name}") do
    ...
    when 'debian'
      shell_out!("/usr/sbin/a2dismod #{new_resource.name}")
    end
  end unless module_disabled?
end
